...
/*void removeRolls()
{
	for(auto chart : mySimfile->charts)
	{
		...
		{
			if(note.type == NOTE_ROLL && note.endrow == note.row + 5)
			{
...
/*void doMagic()
{
	...
	for(; it + 1 != end; ++it)
	{
		double diff = td.rowToTime((it + 1)->row) - td.rowToTime(it->row);
		if(diff > 0)
		{
			//int i = rand() % 3;
			int i = 1;
			double bpm = (60.0 * dists[i] / ROWS_PER_BEAT) / diff;
			if(i == 2 && bpm > 300.0) { --i; bpm *= 2; bpm /= 3; }
			if(i == 1 && bpm > 300.0) { --i; bpm /= 2; }
			if(i == 0 && bpm < 80.0) { ++i; bpm *= 2; }
	...
	for(auto& chart : mySimfile->charts)
	{
		...
		for(auto& note : chart->notes)
		{
			if(note.row != note.endrow)
			{
				note.row = td.timeToRow(timeOffset + *time++);
				note.endrow = td.timeToRow(timeOffset + *time++);
			}
			else
			{
				note.row = note.endrow = td.timeToRow(timeOffset + *time++);
			}
...
void SimfileMan::create()
{
	gSimfile = new SimfileManImpl;
}
